class des():
    def __init__(self):
        self.text = list()
        self.password = list()
        self.PC_1 = [57, 49, 41, 33, 25, 17, 9,
                    1, 58, 50, 42, 34, 26, 18,
                    10, 2, 59, 51, 43, 35, 27,
                    19, 11, 3, 60, 52, 44, 36,
                    63, 55, 47, 39, 31, 23, 15,
                    7, 62, 54, 46, 38, 30, 22,
                    14, 6, 61, 53, 45, 37, 29,
                    21, 13, 5, 28, 20, 12, 4]

        self.PC_2 = [14, 17, 11, 24, 1, 5, 3, 28,
                    15, 6, 21, 10, 23, 19, 12, 4,
                    26, 8, 16, 7, 27, 20, 13, 2,
                    41, 52, 31, 37, 47, 55, 30, 40,
                    51, 45, 33, 48, 44, 49, 39, 56,
                    34, 53, 46, 42, 50, 36, 29, 32]

        self.IP = [58, 50, 42, 34, 26, 18, 10, 2,
                  60, 52, 44, 36, 28, 20, 12, 4,
                  62, 54, 46, 38, 30, 22, 14, 6,
                  64, 56, 48, 40, 32, 24, 16, 8,
                  57, 49, 41, 33, 25, 17, 9, 1,
                  59, 51, 43, 35, 27, 19, 11, 3,
                  61, 53, 45, 37, 29, 21, 13, 5,
                  63, 55, 47, 39, 31, 23, 15, 7]

        self.E =[32, 1, 2, 3, 4, 5, 
                 4, 5, 6, 7, 8, 9,
                 8, 9, 10, 11, 12, 13,
                 12, 13, 14, 15, 16, 17,
                 16, 17, 18, 19, 20, 21,
                 20, 21, 22, 23, 24, 25,
                 24, 25, 26, 27, 28, 29,
                 28, 29, 30, 31, 32, 1]

        self.S_BOX =[
                    [[14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
                     [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
                     [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
                     [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13],
                    ],

                    [[15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
                     [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
                     [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
                     [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9],
                    ],

                    [[10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
                     [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
                     [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
                     [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12],
                    ],

                    [[7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
                     [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
                     [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
                     [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14],
                    ],  

                    [[2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
                     [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
                     [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
                     [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3],
                    ], 

                    [[12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
                     [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
                     [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
                     [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13],
                    ], 

                    [[4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
                     [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
                     [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
                     [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12],
                    ],
   
                    [[13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
                     [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
                     [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
                     [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11],
                     ]
                    ]
        self.G = [57,  49,  41,  33,  25,  17,  9,
                1,  58,  50,  42,  34,  26,  18,
                10,  2,  59,  51,  43,  35,  27,
                19,  11,  3,  60,  52,  44,  36,
                63,  55,  47,  39,  31,  23,  15,
                7,  62,  54,  46,  38,  30,  22,
                14,  6,  61,  53,  45,  37,  29,
                21,  13,  5,  28,  20,  12,  4]

        self.P = [16, 7, 20, 21, 29, 12, 28, 17,
                 1, 15, 23, 26, 5, 18, 31, 10,
                 2, 8, 24, 14, 32, 27, 3, 9,
                 19, 13, 30, 6, 22, 11, 4, 25]

        self.SHIFT = [1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1]

        self.H = [14, 17, 11, 24, 1, 5, 3, 28,
                15, 6, 21, 10, 23, 19, 12, 4,
                26, 8, 16, 7, 27, 20, 13, 2,
                41, 52, 31, 37, 47, 55, 30, 40,
                51, 45, 33, 48, 44, 49, 39, 56,
                34, 53, 46, 42, 50, 36, 29, 32]

    #Permut the given block using the given table
    #Перемешивает блок в соотвествии с таблицей
    def permut(self, block, table):
        return [block[x-1] for x in table]

    """Converting a string into a list of bit"""
    #Преобразует строку в битовую последовательность
    def string_to_byte(self, text):
        string = [bin(symb) if isinstance (symb,int) else bin(ord(symb)) for symb in text]
        bitList = list()
        for byte in string:
            if len(byte) < 9:
                for bit in byte:
                    if bit == 'b': 
                        bit = int('0')
                    bitList.append(bit)
            for bit in byte:
                    if bit == 'b': 
                        bit = int('0')
                    bitList.append(bit)
        return bitList

    """Converting a list of bit into a string"""
    #Преобразует битовую последовательность в строку
    def byte_to_string(self,list_of_byte):
        for strng in list_of_byte:
            text += chr(int(''.join(strng),2))
        return text


    def encrypt(self,text,key):
        btlist = self.string_to_byte(text) #преобразует текст в битовую последовательность
        while len(btlist)//64 != 0: #расширяет битовую последовательность 0, если длина не кратна 64
            btlist.append(int(0))
        bit_block = []
        for btBlock in self.seperation_list(btlist, 64):#разбивает битовую последовательность на 64 битовые блоки
            bit_block.append(self.permut(btBlock, IP))
        self.password = self.string_to_byte(key)[:56]#преобразует пароль в 56 битовую последовательность
        for block in bit_block:
            left,right = seperation_list(block,32)
            left_list = [left]
            right_list = [right]
            for i in range(1,16):
                left_list.append(right_list[i-1])
                right_list.append()


            #for i in range(1,17):
                

    #Разделяет лист на блок листов нужной длины
    def seperation_list(self,lst, step):
        return [lst[x:x+step] for x in range(0,len(lst),step)]

    #Объединяет блоки в листе
    def combine_blocks(self, block):
        cb = []
        for b in block:
            if len(b) > 1:
                for x in b:
                    cb.append(x)
            else:
                cb.append(x)
        return cb

    """возвращает лист [B[0]B[1]..B[8]]"""
    def Feistels_function(self,bit,key):
        new_bit = permut(bit,self.E)
        new_key = key        
        B = self.seperation_list([new_bit[x] ^ new_bit[x] if len(new_bit) == len(new_key) else print('Длина расширенного ключа и длина битов не совпадает') for x in range(len(new_bit))],6)
        #""" операция хор между строкой и ключой и разбиение на список по 6 бит"""
        for i in range(B):
            nb_row = int(''.join(str(b) for b in [[B[i]][0],[B[i]][-1]] ),2)
            nb_column = int(''.join(str(b) for b in [B[i]][1:-1] ),2)
            B[i] = self.string_to_byte([[S_BOX[i]][nb_row]][nb_column])
        return self.permut(self.combine_blocks(B), self.P)
        
    def generate_key(self, key, iteration):
        new_key = permut(key,self.G)
        c,d = seperation_list(new_key,28)
        c,d = self.shift(c,d,SHIFT[iteration])
        return self.permut(c + d, self.H)

    #сдвиг листа на определенное значение
    def shift(self, c, d, n): 
        return c[n:] + c[:n], d[n:] + d[:n]

    #def expand(self,bit,block):
